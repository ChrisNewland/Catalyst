...
Single shared password — volunteers get the volunteer password, admins get the admin password.
...
"Logged by" is required (pre-filled from localStorage on repeat visits), Food = Some, Water = Normal, urinated on, defecated on → Bristol picker appears, 4 selected, Condition = Concern.

![Log form filled](./05-log-form.png)

## 5. Dashboard after saving

Green toast confirms the save; the cat moves from "Needs visit today" to "Visited today" with a "Log again" CTA, attributed to the name typed in the log form.

![Dashboard after log](./06-dashboard-after.png)

## 6. Cat profile — last 7 days

Shows the entry we just saved, the name of whoever recorded it, and a condition chip.

![Cat profile](./07-cat-profile.png)

## 7. Admin dashboard

Same cats view as a volunteer sees, but the header grows an extra "Cats" admin nav link.
